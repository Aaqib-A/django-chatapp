...
class ChatroomConsumer(WebsocketConsumer):
    ...
    def online_count_handler(self, event):
        ...
        chat_messages = ChatGroup.objects.get(group_name=self.chatroom_name).chat_message.all()[:30]
...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status_handler(self, event):
        ...
        if public_chat_users.exists() or private_chats_with_users or group_chats_with_users:
        ...
        html = render_to_string('rt_chat/partials/online_status.html', context=context)
        self.send(text_data=html)
